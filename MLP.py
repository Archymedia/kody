...
HYPERPARAMS = {
    'data': {
        'data_path': "/Users/lindawaisova/DP/data/SP_100.csv",
        'vix_path': "/Users/lindawaisova/DP/data/VIX.csv",
        ...
        'output': {
        'png_path': "/Users/lindawaisova/DP/výsledky/MLP.png",
    },
    'features': {
        'window': 15,
        'use_RSI': True,
    ...
    'model': {
        ...
        'CV_epochs': 15,          # epochs per fold during tuning 
        'final_epochs': 50,       # epochs for final training on full development
...
def build_sequences_for_group(g: pd.DataFrame, h: int, feat_cfg):
    ...
    # Pomocné funkce pro subsekvence
    def subseq_return(t0, t1):  
    ...
    for t in range(h-1, T-1):
        ...
        # Skládání: returns, close_norm, high_rel, low_rel, open_ratio, volume_norm, RSI, CCI, STOCH
...
REQUIRED_TUNED_KEYS = ['hidden_layers','hidden_units','l2','learning_rate']
...
def build_mlp_model(input_shape, hp):
    from tensorflow.keras import regularizers
    lr = float(hp['learning_rate'])
    l2w = float(hp['l2'])
    u = int(hp['hidden_units'])
    n = int(hp['hidden_layers'])
    dr = float(hp['dropout_rate'])

    model = models.Sequential()
    model.add(layers.Input(shape=input_shape))
    for _ in range(n):
        model.add(layers.Dense(u, activation='relu', kernel_regularizer=regularizers.l2(l2w)))
...
def _window_tail_stats(seq_1d: np.ndarray, w: int):
    w = int(max(1, min(w, len(seq_1d))))
    tail = seq_1d[-w:]
    m = float(np.mean(tail))
    s = float(np.std(tail, ddof=1)) if w > 1 else 0.0
    return m, s

def subseq_to_features(X_seq: np.ndarray) -> np.ndarray:
    h, C = X_seq.shape
    feat_list = []
    for c in range(C):
        s = X_seq[:, c]
        last  = float(s[-1])
        last1 = float(s[-2]) if h >= 2 else 0.0
        last2 = float(s[-3]) if h >= 3 else 0.0
        m5,  s5  = _window_tail_stats(s, 5)
        m10, s10 = _window_tail_stats(s, 10)
        mall, sall = _window_tail_stats(s, h)
        feat_list.extend([last, last1, last2, m5, s5, m10, s10, mall, sall])
    return np.array(feat_list, dtype=float)

def make_mlp_features(X: np.ndarray) -> np.ndarray:
    return np.stack([subseq_to_features(xi) for xi in X], axis=0)

def cv_train_mlp(X_dev, y_dev, dates_dev, hp_override=None):
    base = HYPERPARAMS['model'].copy()
    hp = base if hp_override is None else {**base, **hp_override}
    _require_tuned(hp)
    k_folds = int(hp['k_folds'])
    log(f"CV {k_folds}-fold (MLP) pro hp={{'layers': hp['hidden_layers'], 'units': hp['hidden_units'], 'lr': hp['learning_rate'], 'bs': hp['batch_size'], 'l2': hp['l2']}}")
    ...
    for fold_idx in range(k_folds):
        ...
        model = build_mlp_model(input_shape=X_train_n.shape[1:], hp=hp)
    ...
    log(f"=> mean best val_loss across folds (MLP): {mean_best:.6f}")
...
def format_hp_short(hp):
    ...
    return f"L{get('hidden_layers')}×{get('hidden_units')}, lr={get('learning_rate')}, bs={get('batch_size')}, l2={get('l2')}"


def final_train_mlp(X_dev, y_dev, best_hp):
    """Finální trénink MLP na celém developmentu s vybranými hyperparametry."""
    log("Finální trénink MLP na celém developmentu s vybranými hyperparametry...")
    ...
    model = build_mlp_model(input_shape=X_dev_n.shape[1:], hp=hp)
    es = callbacks.EarlyStopping(monitor='val_loss', patience=hp['early_stopping_patience'], min_delta=1e-6, restore_best_weights=True)
    hist = model.fit(
        X_dev_n, y_dev,
    ...
    log(f"Finální trénink hotov (MLP). Nejlepší val_loss={min(hist.history['val_loss']):.6f}")
...
def run_strategy(pred_df, ohlc_map, dates_ordered, top_n=10, bottom_n=10, tp=0.02, sl=-0.02, priority='SL_first', phase=None):
    ...
    # === Denní plán pro otevřené pozice ===
    active_schedule = {}
    ...
    # Předvýběr plánovaných otevření
    planned_openings = {}
...
# === export TEST cumulative strategy series to Excel with CSV fallback ===
def save_test_oos_excel(strat_cum_test: pd.Series, out_paths):
    ...
    """
    xlsx_paths = _derive_sibling_paths(out_paths, "MLP_test_oos_cum.xlsx")
    csv_paths  = _derive_sibling_paths(out_paths, "MLP_test_oos_cum.csv")
...
def plot_dashboard(strat_cum, bench_cum, test_start_date, strat_cum_test, bench_cum_test, final_hist, out_paths, hedged_cum_full=None, hedged_cum_test=None):
    ...
    ax1.legend()
    ax1.set_title("MLP Strategy vs. Benchmark (Cumulative Returns)")
    ax1.set_xlabel("Date")
...
def main():
    t0 = time.time()
    log("Startuji MLP SP100 pipeline...")
    ...
    # --- převod subsekvencí (h×C) na statické featury pro MLP ---
    log("Generuji MLP featury z subsekvencí...")
    t_feat = time.time()
    X_dev = make_mlp_features(X_dev)   # (N, F)
    X_test = make_mlp_features(X_test) # (N, F)
    dt_feat = time.time() - t_feat
    log(f"MLP featury hotovo za {dt_feat:.1f}s")
    log(f"MLP featury: X_dev={X_dev.shape}, X_test={X_test.shape}")


    log(f"Shapes: X_dev={X_dev.shape}, X_test={X_test.shape} ; počet featur={X_dev.shape[-1]} ; feat_names={feat_names}")
    ...
    for cand in make_param_grid(search_space):
        ...
        mean_val_loss, _ = cv_train_mlp(X_dev, y_dev, dates_dev, hp_override=cand_hp)
    ...
    # 5) Finální trénink TOP-5 kandidátů a výběr nejlepšího 
    TOP_K = 5
    topk_df = res_df.head(TOP_K).copy()
    log(f"Retraining TOP-{len(topk_df)} candidates on full development and selecting the best by min(val_loss) during final training...")
    ...
    for i, row in topk_df.reset_index(drop=True).iterrows():
        cand_hp = {**HYPERPARAMS['model'], **{k: row[k] for k in search_space.keys()}}
        log(f"  [Final {i+1}/{len(topk_df)}] {format_hp_short(cand_hp)}: training...")
        model_i, mu_i, sigma_i, hist_i = final_train_mlp(X_dev, y_dev, cand_hp)
        min_vl = float(np.min(hist_i.get('val_loss', [np.inf])))
        finalists.append({'hp': cand_hp, 'min_val_loss': min_vl})
        log(f"     -> min(val_loss) = {min_vl:.6f}")
        if min_vl < best_final['min_val_loss']:
            best_final.update({'hp': cand_hp, 'model': model_i, 'mu': mu_i, 'sigma': sigma_i, 'hist': hist_i, 'min_val_loss': min_vl, 'idx': i})

    # Report finalists summary 
    log("=== FINALISTS (by final-training min val_loss) ===")
    for j, rec in enumerate(sorted(finalists, key=lambda d: d['min_val_loss'])):
        log(f"  #{j+1}: {format_hp_short(rec['hp'])} | min_val_loss = {rec['min_val_loss']:.6f}")

    # Use the best finalist 
    best_hp = best_final['hp']
    model, mu_final, sigma_final, final_hist = best_final['model'], best_final['mu'], best_final['sigma'], best_final['hist']
    log(f"=> SELECTED FINAL MODEL: {format_hp_short(best_hp)} | min(val_loss) = {best_final['min_val_loss']:.6f}")
    ...
    # Realizovaná alfa na TEST (fresh-only) a hedged metriky, aby byly k dispozici pro výpis níže 
    ...
    # === POUZE OUT-OF-SAMPLE TEST PERIOD === 
    test_metrics_tbl = {
    ...
    strat_cum_full = (1 + pnl_all).cumprod() - 1.0   
    strat_cum_test = (1 + pnl_test_rebased).cumprod() - 1.0  
    ...
    loss_paths = _derive_sibling_paths(base_paths, "MLP_loss_curve.png")
    test_paths = _derive_sibling_paths(base_paths, "MLP_test_panel.png")
    ...
    log(f"Vzorky: dev={len(pred_dev_df):,}, test={len(pred_test_df):,}, okno h={HYPERPARAMS['features']['window']}, počet featur={X_dev.shape[-1]}")
