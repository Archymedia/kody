...
HYPERPARAMS = {
    'data': {
        ...
        'output': {
        'png_path': "/Users/lindawaisova/DP/výsledky/RNN.png",
    },
    'features': {
        'window': 15,           
        'use_RSI': True,
...
REQUIRED_TUNED_KEYS = ['rnn_units','dense_units','l2']
...
def build_rnn_model(input_shape, hp):
    ...
    model = models.Sequential([
        ...
        layers.SimpleRNN(ru, activation='tanh', dropout=dr, recurrent_dropout=0.0,
...
def cv_train_rnn(X_dev, y_dev, dates_dev, hp_override=None):
    ...
    hp_view = {key: hp[key] for key in ['rnn_units','dense_units','learning_rate','batch_size','l2']}
    log(f"CV {k_folds}-fold pro hp={hp_view}")
    ...
    for fold_idx in range(k_folds):
        ...
        model = build_rnn_model(input_shape=X_train_n.shape[1:], hp=hp)
...
def format_hp_short(hp):
    ...
    return f"RNN={get('rnn_units')}, Dense={get('dense_units')}, lr={get('learning_rate')}, bs={get('batch_size')}, l2={get('l2')}"

def final_train_rnn(X_dev, y_dev, best_hp):
    log("Finální trénink RNN na celém developmentu s vybranými hyperparametry...")
    hp = {**HYPERPARAMS['model'], **best_hp}
    _require_tuned(hp)
    mu, sigma = compute_mu_sigma(X_dev)
    X_dev_n = standardize_with(X_dev, mu, sigma)

    model = build_rnn_model(input_shape=X_dev_n.shape[1:], hp=hp)
    es = callbacks.EarlyStopping(monitor='val_loss', patience=hp['early_stopping_patience'], min_delta=1e-6, restore_best_weights=True)
    hist = model.fit(
        X_dev_n, y_dev,
...
def save_test_oos_excel(strat_cum_test: pd.Series, out_paths):
    xlsx_paths = _derive_sibling_paths(out_paths, "RNN_test_oos_cum.xlsx")
    csv_paths  = _derive_sibling_paths(out_paths, "RNN_test_oos_cum.csv")
...
def plot_dashboard(strat_cum, bench_cum, test_start_date, strat_cum_test, bench_cum_test, final_hist, out_paths, hedged_cum_full=None, hedged_cum_test=None):
    ...
    ax1.legend()
    ax1.set_title("RNN Strategy vs. Benchmark (Cumulative Returns)")
    ax1.set_xlabel("Date")
...
def main():
    t0 = time.time()
    log("Startuji RNN SP100 pipeline...")
    ...
    for cand in make_param_grid(search_space):
        ...
        mean_val_loss, _ = cv_train_rnn(X_dev, y_dev, dates_dev, hp_override=cand_hp)
    ...
    log(f"=> BEST (CV): {best_hp_str:<48} | mean_val_loss = {best_score:.6f}")

    # 5) Finální trénink – TOP-5 kandidátů podle CV a výběr nejlepšího dle val_loss z finálního tréninku
    top_k = 5
    top5_df = res_df.head(min(top_k, len(res_df))).copy()
    log(f"=== FINAL TRAIN (TOP-{len(top5_df)}) kandidátů podle CV ===")

    best_final_model = None
    best_final_mu = None
    best_final_sigma = None
    best_final_hist = None
    best_final_hp = None
    best_final_val = np.inf

    for i, row in enumerate(top5_df.itertuples(index=False), start=1):
        cand_hp = {
            'rnn_units': int(getattr(row, 'rnn_units')),
            'dense_units': int(getattr(row, 'dense_units')),
            'learning_rate': float(getattr(row, 'learning_rate')),
            'l2': float(getattr(row, 'l2')),
        }
        log(f"--- Kandidát {i}/{len(top5_df)}: {format_hp_short(cand_hp)} ---")
        model_i, mu_i, sigma_i, hist_i = final_train_rnn(X_dev, y_dev, cand_hp)
        val_best_i = float(min(hist_i['val_loss'])) if ('val_loss' in hist_i and len(hist_i['val_loss'])>0) else np.inf
        log(f"    -> Final training best val_loss = {val_best_i:.6f}")
        if val_best_i < best_final_val:
            best_final_val = val_best_i
            best_final_model = model_i
            best_final_mu = mu_i
            best_final_sigma = sigma_i
            best_final_hist = hist_i
            best_final_hp = cand_hp

    # Použije nejlepšího z TOP-5 pro zbytek pipeline
    if best_final_hp is None:
        best_hp = {
            'rnn_units': int(best_hp['rnn_units']),
            'dense_units': int(best_hp['dense_units']),
            'learning_rate': float(best_hp['learning_rate']),
            'l2': float(best_hp['l2']),
        }
        log(f"[WARN] TOP-5 výběr selhal, používám TOP-1 dle CV: {format_hp_short(best_hp)}")
        model, mu_final, sigma_final, final_hist = final_train_rnn(X_dev, y_dev, best_hp)
        selected_hp = best_hp
    else:
        model = best_final_model
        mu_final = best_final_mu
        sigma_final = best_final_sigma
        final_hist = best_final_hist
        selected_hp = best_final_hp

    used_final_epochs = HYPERPARAMS['model']['final_epochs']
    used_patience = HYPERPARAMS['model']['early_stopping_patience']
    log(f"=== SELECTED after TOP-{len(top5_df)} final trainings ===")
    log(f"Selected (final): {format_hp_short(selected_hp)} | best_val_loss_final={best_final_val:.6f}")
    best_hp = selected_hp
    ...
    # Sharpe pro OUT-OF-SAMPLE test-only běh 
    ...
    loss_paths = _derive_sibling_paths(base_paths, "RNN_loss_curve.png")
    test_paths = _derive_sibling_paths(base_paths, "RNN_test_panel.png")

    saved_loss = save_loss_curve(final_hist, loss_paths)
    if saved_loss:
        log(f"Uloženo PNG (RNN loss): {saved_loss}")
    else:
        log("Nepodařilo se uložit RNN loss graf.")

    saved_test = save_test_panel(strat_cum_test, bench_cum_test, hedged_cum_test, test_paths)
    if saved_test:
        log(f"Uloženo PNG (RNN test panel): {saved_test}")
    else:
        log("Nepodařilo se uložit RNN test panel graf.")
    ...
    log(f"Training controls: k_folds={HYPERPARAMS['model']['k_folds']}, CV_epochs={HYPERPARAMS['model']['CV_epochs']}, final_epochs={used_final_epochs}, ES_patience={used_patience}, keras_verbose={HYPERPARAMS['model'].get('keras_verbose', 1)}")
