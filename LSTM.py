...
def main():
    ...
    # 5) Finální trénink – TOP-5 re-train (50 epochs, patience 10) na celém developmentu
    log("=== TOP-5 RE-TRAIN (final_epochs=50, patience=10) ===")
    ...
    for i, r in top5.iterrows():
        cand = {
            ...
            'dense_units': int(r['dense_units']),
            'learning_rate': float(r['learning_rate']),
            'l2': float(r['l2']),
            'final_epochs': 50,
            'early_stopping_patience': 10,
        }
        hp_str = format_hp_short(cand)
        log(f"  Re-train {hp_str} ...")
        model_c, mu_c, sigma_c, hist_c = final_train_lstm(X_dev, y_dev, cand)
        if 'val_loss' in hist_c and len(hist_c['val_loss']) > 0:
            best_val = float(np.min(hist_c['val_loss']))
        else:
            best_val = float(np.inf)
        top5_results.append({
            'lstm_units': cand['lstm_units'],
            'dense_units': cand['dense_units'],
            'learning_rate': cand['learning_rate'],
            'l2': cand['l2'],
            'final_val_loss': best_val
        })
        log(f"    -> best val_loss = {best_val:.6f}")
        if best_val < best_final_val:
            best_final_val = best_val
            best_hp_final = cand
            best_bundle = (model_c, mu_c, sigma_c, hist_c)

    top5_res_df = pd.DataFrame(top5_results).sort_values('final_val_loss')
    log("=== TOP-5 FINAL TRAIN RESULTS (ascending by best val_loss) ===")
    for _, rr in top5_res_df.iterrows():
        hp_line = f"LSTM={int(rr['lstm_units'])}, Dense={int(rr['dense_units'])}, lr={rr['learning_rate']}, l2={rr['l2']}"
        log(f"  {hp_line:<48} | best val_loss = {rr['final_val_loss']:.6f}")

    # Zvol nejlepší z TOP-5 pro další kroky
    best_hp = best_hp_final
    model, mu_final, sigma_final, final_hist = best_bundle
    used_final_epochs = 50
    used_patience = 10
    ...
    # Rozdělení trades pro obchodní metriky
