...
HYPERPARAMS = {
    ...
    'model': {
        ...
        'num_blocks': 3,
        'kernel_size': 5,
        'dropout_rate': 0.1,     # v konvolučních blocích
        'final_dropout': 0.2,
        'batch_size': 64,
        'n_best_final': 5,  
        # Tuning 
...
def load_data():
    ...
    df.sort_values([cfg['ticker_col'], cfg['date_col']], inplace=True)
...
def compute_IDContIndex(df: pd.DataFrame):
    ...
    """
    cfg = HYPERPARAMS['data']
    id_col, date_col = cfg['ticker_col'], cfg['date_col']
...
REQUIRED_TUNED_KEYS = ['filters','dense_units','l2','learning_rate']
...
def build_cnn_model(input_shape, hp):
    from tensorflow.keras import regularizers
    lr = float(hp['learning_rate'])
    l2w = float(hp['l2'])
    du = int(hp['dense_units'])
    lu = int(hp['filters'])  
    # 1D CNN (input shape: (h, C)) – blocks: [Conv1D -> BN -> ReLU -> SpatialDropout]
    num_blocks = int(hp.get('num_blocks', 3))
    ksize = int(hp.get('kernel_size', 5))
    conv_dr = float(hp.get('dropout_rate', 0.1))
    final_dr = float(hp.get('final_dropout', 0.2))

    model = models.Sequential()
    model.add(layers.Input(shape=input_shape))
    for _ in range(num_blocks):
        model.add(layers.Conv1D(filters=lu, kernel_size=ksize, padding='causal', use_bias=False,
                                kernel_regularizer=regularizers.l2(l2w)))
        model.add(layers.BatchNormalization())
        model.add(layers.ReLU())
        model.add(layers.SpatialDropout1D(conv_dr))

    model.add(layers.GlobalAveragePooling1D())
    model.add(layers.Dense(du, activation='relu', kernel_regularizer=regularizers.l2(l2w)))
    model.add(layers.Dropout(final_dr))
...
def cv_train_cnn(X_dev, y_dev, dates_dev, hp_override=None):
    ...
    log(f"CV {k_folds}-fold pro hp={ {k: hp[k] for k in ['filters','dense_units','learning_rate','batch_size','l2']} }")
    ...
    for fold_idx in range(k_folds):
        ...
        model = build_cnn_model(input_shape=X_train_n.shape[1:], hp=hp)
...
def format_hp_short(hp):
    ...
    return f"Filters={get('filters')}, Dense={get('dense_units')}, lr={get('learning_rate')}, bs={get('batch_size')}, l2={get('l2')}"

def final_train_cnn(X_dev, y_dev, best_hp):
    log("Finální trénink CNN na celém developmentu s vybranými hyperparametry...")
    hp = {**HYPERPARAMS['model'], **best_hp}
    _require_tuned(hp)
    mu, sigma = compute_mu_sigma(X_dev)
    X_dev_n = standardize_with(X_dev, mu, sigma)

    model = build_cnn_model(input_shape=X_dev_n.shape[1:], hp=hp)
    es = callbacks.EarlyStopping(monitor='val_loss', patience=hp['early_stopping_patience'], min_delta=1e-6, restore_best_weights=True)
    hist = model.fit(
        X_dev_n, y_dev,
...
def run_strategy(pred_df, ohlc_map, dates_ordered, top_n=10, bottom_n=10, tp=0.02, sl=-0.02, priority='SL_first', phase=None):
    ...
    for d in all_dates:
        ...
        # 2) Otevři nové pozice podle plánu
...
def save_test_oos_excel(strat_cum_test: pd.Series, out_paths):
    ...
    """
    xlsx_paths = _derive_sibling_paths(out_paths, "CNN_test_oos_cum.xlsx")
    csv_paths  = _derive_sibling_paths(out_paths, "CNN_test_oos_cum.csv")
...
def plot_dashboard(strat_cum, bench_cum, test_start_date, strat_cum_test, bench_cum_test, final_hist, out_paths, hedged_cum_full=None, hedged_cum_test=None):
    ...
    ax1.legend()
    ax1.set_title("CNN Strategy vs. Benchmark (Cumulative Returns)")
    ax1.set_xlabel("Date")
...
def main():
    t0 = time.time()
    log("Startuji CNN SP100 pipeline...")
    ...
    for cand in make_param_grid(search_space):
        ...
        mean_val_loss, _ = cv_train_cnn(X_dev, y_dev, dates_dev, hp_override=cand_hp)
    ...
    # 5) Finální trénink: vezmeme TOP-5 kandidátů podle mean_val_loss
    n_best = int(HYPERPARAMS['model'].get('n_best_final', 1))
    top_candidates = res_df.head(max(1, n_best)).copy()

    finalists = []  
    log(f"Spouštím finální trénink pro TOP-{len(top_candidates)} kandidátů...")
    for i, (_, cand_row) in enumerate(top_candidates.iterrows(), start=1):
        cand_hp = cand_row.drop(labels=['mean_val_loss']).to_dict()
        log(f"  [{i}/{len(top_candidates)}] Finální trénink pro {format_hp_short(cand_hp)}")
        model_i, mu_i, sigma_i, hist_i = final_train_cnn(X_dev, y_dev, cand_hp)
        if hist_i is not None and 'val_loss' in hist_i and len(hist_i['val_loss']) > 0:
            val_best_i = float(np.min(hist_i['val_loss']))
        else:
            val_best_i = float(np.min(hist_i['loss'])) if (hist_i is not None and 'loss' in hist_i) else np.inf
        log(f"    → Final best val_loss = {val_best_i:.6f}")
        finalists.append((val_best_i, model_i, mu_i, sigma_i, hist_i, cand_hp))

    # výběr nejlepšího finalisty podle nejnižší final val_loss
    finalists.sort(key=lambda t: t[0])
    best_final_val, model, mu_final, sigma_final, final_hist, best_hp = finalists[0]
    log(f"=> Nejlepší z finálního tréninku: {format_hp_short(best_hp)} | final best val_loss = {best_final_val:.6f}")
    ...
    pnl_all, trades_all = run_strategy(
        ...
        tp=strat_cfg['tp_pct'], sl=-strat_cfg['sl_pct'], priority=strat_cfg['priority'],
        phase='full'
    )
    ...
    loss_paths = _derive_sibling_paths(base_paths, "CNN_loss_curve.png")
    test_paths = _derive_sibling_paths(base_paths, "CNN_test_panel.png")
